...
class MyModel_p(nn.Module):
    def __init__(self, args, config):
        super(MyModel_p, self).__init__()
        self.args = args
        self.config = config
        ...
        self.head = nn.Sequential(
            ...
            nn.Linear(emb_d, args.feat_dim)
        )
    def forward(self, data):
        score, pos_score, neg_score = self.separator(data)

        c_logit, c_graph_feat, s_graph_feat, cmt_loss = self.encoder(data, None)
        loss_reg = torch.abs(pos_score / (pos_score + neg_score) - self.args.gamma * torch.ones_like(pos_score)).mean()

        return self.head(c_graph_feat),self.head(s_graph_feat),  c_graph_feat,s_graph_feat,loss_reg
